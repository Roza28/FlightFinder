...
class MultiGraph
{
    ...
    public:
    //HELPERS ADDED BY ME
    int search_vertex(std::string vertexName) const ;
    int search_edge(std::string edgeName,GraphVertex vertex,int IndexTo) const ;
    
    //CENGFlight Functions
    float get_weight_w0(std::string edgeName,int from, int to) const;
    float get_weight_w1(std::string edgeName,int from, int to) const;
    std::vector<GraphVertex> get_vertexList() const {return vertexList;};
